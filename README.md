...
可能的几种情况
 ...
 若有Android root权限，在Termux内输入pkg install tsu,安装完成后输入'tsu'，让Termux应用获取root权限即可。
...
若repo目录内包含requirements.txt,直接pip install -r requirements.txt即可
 ...
 * 安装报错？
 ...
 [1]使用pip download $ModuleName 下载tar.gz文件
 ...
 * 注意
 ...
 有时repo的启动脚本没有.rb这个后缀，可在$PREFIX/bin中建立它的软链接方便使用。
 ...
 * 常见问题
 ...
 C库依赖……仍用运行发行版后apt install ruby-$module解决。官方有rbnacl-libsodium的补丁，可以自行patch后安装(但要在之前自行编译ruby2.4并安装)。
 
 bundler对tmp目录无写权限。chmod 777是不可行的，要用root权限来运行bundler。
 
 * nodejs
 
 node xxx.js  cd $repodir&&npm install
 ...
 * 何为shebang?
 ...
 或用termux-fix-shebang $script 修改脚本shebang。
 ...
 * go
 ...
 go get $repourl ,注意去掉$repourl的'https://'前缀。
 
 完成后在$HOME建立go工作目录，在其中的bin子目录有编译完成的elf可执行文件。
 
 或者cd $repodir&&go build
